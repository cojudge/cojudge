<script lang="ts">
	...
	/** Safari trackpad pinch baseline (board zoom at gesturestart). */
	let gestureBaseZoom = 1;
	let gestureActive = false;

	function clientPointOverBoard(clientX: number, clientY: number): boolean {
	...
	function applyCanvasZoomAt(clientX: number, clientY: number, nextZoom: number): void {
		const rect = canvasElement?.getBoundingClientRect();
		if (!rect) return;
		const local = { x: clientX - rect.left, y: clientY - rect.top };
		const world = { x: (local.x - panX) / zoom, y: (local.y - panY) / zoom };
		const z = clamp(nextZoom, MIN_ZOOM, MAX_ZOOM);
		panX = local.x - world.x * z;
		panY = local.y - world.y * z;
		zoom = z;
		saveBoardSoon();
	}

	// Safari: convert gesture pinch into canvas zoom (and block page zoom).
	function handleGestureStart(event: Event): void {
		...
		// Prefer event target so overlays above the board are not stolen; fall back
		// to geometry only when the target is not a Node.
		if (ge.target instanceof Node) {
			if (!isEventInsideBoard(ge.target)) return;
		} else if (!clientPointOverBoard(ge.clientX, ge.clientY)) {
			return;
		}
		ge.preventDefault();
		gestureActive = true;
	...
	function handleGestureChange(event: Event): void {
		...
		const ge = event as unknown as { clientX: number; clientY: number; scale: number; preventDefault: () => void };
		ge.preventDefault();
		applyCanvasZoomAt(ge.clientX, ge.clientY, gestureBaseZoom * (ge.scale || 1));
	}
	...
	onMount(() => {
		...
		// Capture on the window so macOS trackpad pinch (ctrl+wheel) zooms the
		// canvas and cannot fall through to browser page-zoom when embedded.
		window.addEventListener('wheel', handleWheel, { capture: true, passive: false });
		document.addEventListener('gesturestart', handleGestureStart, { capture: true, passive: false } as AddEventListenerOptions);
		document.addEventListener('gesturechange', handleGestureChange, { capture: true, passive: false } as AddEventListenerOptions);
		document.addEventListener('gestureend', handleGestureEnd, { capture: true, passive: false } as AddEventListenerOptions);
		...
		return () => {
			...
			document.removeEventListener('gestureend', handleGestureEnd, { capture: true } as EventListenerOptions);
	...
	function isEventInsideBoard(target: EventTarget | null): boolean {
		return !!boardRoot && target instanceof Node && boardRoot.contains(target);
	}

	function handleWheel(event: WheelEvent): void {
		// Only handle wheels that actually target the whiteboard tree. Do not use
		// geometric hit-testing — overlays like CloudSyncModal sit above the board
		// and must keep their own scroll.
		if (!active || !isEventInsideBoard(event.target)) return;

		const target = event.target;
		const overScrollableUi =
			target instanceof Element
			&& !!target.closest(
				'.library-panel, .style-panel, .popover, .modal-card, .modal-shell, .top-toolbar, textarea, input, [data-scrollable]'
			);

		// Trackpad pinch arrives as ctrl/meta + wheel — zoom the canvas only.
		if (event.ctrlKey || event.metaKey) {
			event.preventDefault();
			event.stopPropagation();
			const nextZoom = zoom * Math.exp(-deltaYFor(event) * 0.005);
			applyCanvasZoomAt(event.clientX, event.clientY, nextZoom);
			return;
		}

		// Let nested scrollable chrome scroll normally.
		if (overScrollableUi) return;

		event.preventDefault();
		event.stopPropagation();
		const deltaX = deltaXFor(event);
		const deltaY = deltaYFor(event);
		panX -= event.shiftKey && deltaX === 0 ? deltaY : deltaX;
		panY -= event.shiftKey ? deltaX : deltaY;
		saveBoardSoon();
	}

	function wheelDeltaMultiplier(event: WheelEvent): number {
		if (event.deltaMode === WheelEvent.DOM_DELTA_LINE) return 16;
		if (event.deltaMode === WheelEvent.DOM_DELTA_PAGE) {
			return canvasElement?.clientHeight ?? window.innerHeight;
		}
		return 1;
	}

	function deltaXFor(event: WheelEvent): number {
		return event.deltaX * wheelDeltaMultiplier(event);
	}

	function deltaYFor(event: WheelEvent): number {
		return event.deltaY * wheelDeltaMultiplier(event);
	}
...
<main class:dark={isDark} class="whiteboard-page" class:embedded bind:this={boardRoot}>
	<div
		class:grid={showGrid}
...
<style>
	:global(html:has(.whiteboard-page:not(.embedded))) {
		overflow: hidden;
